...
void OdometryThread::Run(units::millisecond_t period) {
  ...
  while (!m_exit_flag) {
    ...
    bool timed_out =
        SwerveModule::SignalGroup::WaitForAllSignals(period, m_moduleSignals);
    ...
    if (now - timestamp - period > 0.5 * period || timed_out) {
      ...
      if (warning_count++ % 20 == 0) {
        ...
        frc::SmartDashboard::PutNumber(
            "Swerve/odom_thread/average loop time (ms)", average_time.value());

        frc::SmartDashboard::PutNumber("Swerve/odom_thread/percent timeout",
                                       100.0 * warning_count / total);
      }
    }

    timestamp = now;
    PutData(m_odom.GetPose(), kDriveKinematics.ToChassisSpeeds(each_state()),
            timestamp);
  }
}
